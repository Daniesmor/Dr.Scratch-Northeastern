...
class DeadCode(Plugin):
    ...
    def store_script(self, dicc, blocks):
        ...
        blocks_set = set(self.blocks_list)

        if script_text not in blocks_set:
            self.blocks_list.append(script_text)  
            self.dict_babia[self.sprite][f'script_{self.currScript}'] = len(clean_script_text)
    ...
    def _clear_resources(self):
        ...
        """
        self.dict_mastery = None
        self.dict_babia = None
    ...
    def finalize(self):
        ...
        self.dict_mastery['list_dead_code_scripts'] = [self.dict_deadcode.copy()]

        dict_result = {'plugin': 'dead_code', 'result': self.dict_mastery.copy(), 'babia': self.dict_babia.copy()}
